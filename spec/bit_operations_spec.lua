...
-- Set up vim mock before loading bytelocker
...
describe("Bit Operations", function()
    describe("rol8 (rotate left 8-bit)", function()
        it("should rotate 0x01 left by 1 bit to 0x02", function()
            assert.are.equal(0x02, bl.rol8(0x01, 1))
        end)

        it("should rotate 0x01 left by 4 bits to 0x10", function()
            assert.are.equal(0x10, bl.rol8(0x01, 4))
        end)

        it("should rotate 0x80 left by 1 bit to 0x01 (wrap around)", function()
            assert.are.equal(0x01, bl.rol8(0x80, 1))
        end)

        it("should rotate 0xFF left by any amount to 0xFF", function()
            assert.are.equal(0xFF, bl.rol8(0xFF, 1))
            assert.are.equal(0xFF, bl.rol8(0xFF, 4))
            assert.are.equal(0xFF, bl.rol8(0xFF, 7))
        end)

        it("should handle 0x00 correctly (always 0)", function()
            assert.are.equal(0x00, bl.rol8(0x00, 1))
            assert.are.equal(0x00, bl.rol8(0x00, 7))
        end)

        it("should handle rotation by 0 (identity)", function()
            assert.are.equal(0x42, bl.rol8(0x42, 0))
            assert.are.equal(0xAB, bl.rol8(0xAB, 0))
        end)

        it("should handle rotation by 8 (full cycle = identity)", function()
            assert.are.equal(0x42, bl.rol8(0x42, 8))
            assert.are.equal(0xCD, bl.rol8(0xCD, 8))
        end)

        it("should handle rotation by more than 8 (modulo)", function()
            -- 9 % 8 = 1, so rol8(x, 9) == rol8(x, 1)
            assert.are.equal(bl.rol8(0x55, 1), bl.rol8(0x55, 9))
            assert.are.equal(bl.rol8(0xAA, 3), bl.rol8(0xAA, 11))
        end)

        it("should rotate 0xAA (10101010) left by 1 to 0x55 (01010101)", function()
            assert.are.equal(0x55, bl.rol8(0xAA, 1))
        end)

        it("should mask values > 255 to 8-bit", function()
            -- Input of 0x1FF should be masked to 0xFF
            assert.are.equal(bl.rol8(0xFF, 1), bl.rol8(0x1FF, 1))
        end)
        ...
        it("should be reversible with ror8", function()
            for value = 0, 255, 17 do  -- Sample some values
                for bits = 0, 7 do
                    local rotated = bl.rol8(value, bits)
                    local restored = bl.ror8(rotated, bits)
    ...
    describe("ror8 (rotate right 8-bit)", function()
        it("should rotate 0x02 right by 1 bit to 0x01", function()
            assert.are.equal(0x01, bl.ror8(0x02, 1))
        end)

        it("should rotate 0x10 right by 4 bits to 0x01", function()
            assert.are.equal(0x01, bl.ror8(0x10, 4))
        end)

        it("should rotate 0x01 right by 1 bit to 0x80 (wrap around)", function()
            assert.are.equal(0x80, bl.ror8(0x01, 1))
        end)

        it("should rotate 0xFF right by any amount to 0xFF", function()
            assert.are.equal(0xFF, bl.ror8(0xFF, 1))
            assert.are.equal(0xFF, bl.ror8(0xFF, 4))
            assert.are.equal(0xFF, bl.ror8(0xFF, 7))
        end)

        it("should handle 0x00 correctly (always 0)", function()
            assert.are.equal(0x00, bl.ror8(0x00, 1))
            assert.are.equal(0x00, bl.ror8(0x00, 7))
        end)

        it("should handle rotation by 0 (identity)", function()
            assert.are.equal(0x42, bl.ror8(0x42, 0))
            assert.are.equal(0xAB, bl.ror8(0xAB, 0))
        end)

        it("should handle rotation by 8 (full cycle = identity)", function()
            assert.are.equal(0x42, bl.ror8(0x42, 8))
            assert.are.equal(0xCD, bl.ror8(0xCD, 8))
        end)

        it("should rotate 0x55 (01010101) right by 1 to 0xAA (10101010)", function()
            assert.are.equal(0xAA, bl.ror8(0x55, 1))
        end)
        ...
        it("should be reversible with rol8", function()
            for value = 0, 255, 17 do
                for bits = 0, 7 do
                    local rotated = bl.ror8(value, bits)
                    local restored = bl.rol8(rotated, bits)
                    assert.are.equal(value, restored,
                        string.format("Failed for value=%d, bits=%d", value, bits))
                end
            end
    ...
    describe("combined rotation properties", function()
        it("rol8 by n equals ror8 by (8-n)", function()
            for value = 0, 255, 31 do
                for bits = 0, 7 do
                    local left_rotated = bl.rol8(value, bits)
                    local right_rotated = bl.ror8(value, 8 - bits)
